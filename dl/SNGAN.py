...
class SNGAN(object):
    def __init__(
        ...
        self.global_step = 0 # tf.Variable(0, name='global_step', trainable=False)
        ...
        # build loss
        self.d_loss = tf.reduce_mean(tf.nn.softplus(-d_x)) + tf.reduce_mean(tf.nn.softplus(d_g_z))
        self.g_loss = tf.reduce_mean(tf.nn.softplus(-d_g_z))
    ...
    def build_discriminator_graph(self, inputs, reuse=None):
        with tf.variable_scope('discriminator', reuse=reuse):
            nn = inputs
            for idx in range(2):
                nn = res_block_down(nn, filter_size=(3, 3), n_filters=128,
                                    scope=f'res_down{idx}')
            for idx in range(2):
                nn = res_block(nn, filter_size=(3, 3), n_filters=128,
                               scope=f'res_{idx}')
            nn = tf.nn.relu(nn)
    ...
    def show_progress(self, n_samples=1, save_dir='./assets/SNGAN/'):
        # print(f'at global step {gs}')
        ...
        if save_dir is not None:
            save_images(images, prefix=f'{self.global_step}_', save_dir=save_dir)
    ...
    def test_step(self):
        return get_inception_score(self.data_test)